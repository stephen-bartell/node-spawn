...
function Spawn (options) {
  ...
  this.id = this.options.id = uuid.v1()
}

// inherit the eventemitter
util.inherits(Spawn, events.EventEmitter2)
...
Spawn.prototype.start = function () {
  ...
  function run (cb) {
    ...
    self.emit(['start', options.id], options)
    ...
    _spawn.on('exit', function (code) {
      ...
      if (options._kill) {
        ...
        self.emit(['dead', options.id], 'no more restarts', options)
      ...
      // act on controls
      if (doRestart) {
        self.emit(['restart', options.id], options)
